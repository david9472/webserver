...
void SocketMessageQueue::enqueueReceivedMessage(const Message& message)
{
...
Message SocketMessageQueue::retrieveReceivedMessage()
{
  const logging::Trace trace(__func__ );
  ...
  const Message message{received_queue_.front()};
  received_queue_.pop();
...
std::optional<Message> SocketMessageQueue::retrieveResponseMessageNonBlocking()
{
  const logging::Trace trace(__func__ );
  ...
  Message response{respond_queue_.front()};
  respond_queue_.pop();

  return response;
}

Message SocketMessageQueue::retrieveResponseMessage()
{
  const logging::Trace trace(__func__ );
  ...
  const Message response{respond_queue_.front()};
  respond_queue_.pop();
...
void SocketMessageQueue::enqueueResponseMessage(const Message& message)
{
  const logging::Trace trace(__func__ );
